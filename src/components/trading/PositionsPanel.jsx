  ...
  return (
    <Box sx={{ bgcolor: '#06060f', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Box sx={{ flex: 1, overflow: 'auto' }}>
        {tab === 0 && <PositionsTable rows={positions} />}
        {tab === 1 && <OpenOrdersTable rows={openOrders} onCancel={handleCancel} />}
        {tab === 2 && <HistoryTable rows={history} />}
      </Box>
    </Box>
  );
});
...
const PositionsTable = memo(function PositionsTable({ rows }) {
  ...
  return (
    <Table size="small" stickyHeader>
      <TableHead>
        <TableRow>
          {['Symbol','Side','Size','Entry','Mark','Liq.','Unrealised PnL','ROE%','TP','SL'].map(h =>
...
const OpenOrdersTable = memo(function OpenOrdersTable({ rows, onCancel }) {
  ...
  return (
    <Table size="small" stickyHeader>
      <TableHead>
        <TableRow>
          {['Symbol','Side','Type','Price','Qty','Filled','TP','SL','Status','Action'].map(h =>
            <TableCell key={h} sx={hdr}>{h}</TableCell>)}
        </TableRow>
      </TableHead>
      <TableBody>
        {rows.map((o, i) => (
          <TableRow key={i} sx={{ '&:hover': { bgcolor: '#ffffff04' } }}>
            ...
            <TableCell sx={{ ...cel, color: '#f6465d' }}>{o.stopLoss || '—'}</TableCell>
            <TableCell sx={cel}>
...
const HistoryTable = memo(function HistoryTable({ rows }) {
  ...
  return (
    <Table size="small" stickyHeader>
      <TableHead>
        <TableRow>
          {['Symbol','Side','Type','Avg Price','Qty','Status','TP','SL','Time'].map(h =>
            <TableCell key={h} sx={hdr}>{h}</TableCell>)}
        </TableRow>
      </TableHead>
      <TableBody>
        {rows.slice(0, 100).map((o, i) => (
          <TableRow key={i} sx={{ '&:hover': { bgcolor: '#ffffff04' } }}>
            ...
            <TableCell sx={{ ...cel, color: '#00d98b' }}>{o.takeProfit || '—'}</TableCell>
            <TableCell sx={{ ...cel, color: '#f6465d' }}>{o.stopLoss || '—'}</TableCell>
            <TableCell sx={cel}>{o.createdTime ? new Date(parseInt(o.createdTime)).toLocaleString() : '—'}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
